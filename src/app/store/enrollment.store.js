...
export const EnrollmentStore = signalStore({ providedIn: 'root' }, withState({
    isLoading: false,
    error: null,
}), withEntities(), withComputed((store) => ({
    ...
    loadEnrollments: rxMethod(pipe(tap(() => patchState(store, { isLoading: true, error: null })), concatMap(() => api.getAll().pipe(tap((rows) => patchState(store, setAllEntities(rows), { isLoading: false })), catchError((err) => {
        patchState(store, {
            isLoading: false,
    ...
    // Optimistic Approve
    approveEnrollment: rxMethod(pipe(tap((id) => {
        patchState(store, updateEntity({ id, changes: { status: 'Approved' } }));
    }), concatMap((id) => api.approve(id).pipe(catchError((err) => {
    ...
    // Listen for SignalR live updates
    listenForLiveUpdates: rxMethod(pipe(tap(() => sync.connect()), switchMap(() => sync.events$), tap((event) => {
